...
def detect_attack(pkt):
    ...
    if pkt.haslayer(scapy.all.ICMP):
        proto = "ICMP"
        src_port = 0
        dst_port = 0
        flag = "-"
        service = "echo"
        # ICMP packets should not be blocked, so just log them
    ...
    # Only block if attack is detected AND it's not ICMP
    if attack_type != "Normal" and proto != "ICMP":
...
def block_ip(ip):
    ...
    try:
        # Block TCP and UDP traffic, but allow ICMP
        command = f"sudo ufw deny proto tcp from {ip} && sudo ufw deny proto udp from {ip}"
        subprocess.run(command, shell=True, check=True)
        console_alert(f"Blocked IP {ip} (except ICMP) due to detected attack", "WARNING")
